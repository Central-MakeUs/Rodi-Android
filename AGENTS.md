...
- Contract는 화면마다 `XxxViewModel.kt` 옆 `XxxContract.kt` 하나로 두고 채택한 UiState·Intent·Effect를 여기에 선언한다.
  Intent·Effect가 필요 없는 화면에 빈 타입이나 stream을 만들지 않는다(`docs/conventions/mvi.md`)
