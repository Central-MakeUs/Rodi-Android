...
internal object DrivingNotificationFactory {
    ...
    fun ongoing(
        ...
        val progress = (session.requiredDistanceMeters ?: session.plannedDistanceMeters)
            ?.takeIf { it > 0 }
            ?.let { required ->
                ((traveledDistanceMeters / required) * PROGRESS_MAX).toInt().coerceIn(0, 99)
            }
        val isInCourseScope = session.isInCourseScope
        val title = if (isInCourseScope) {
            "코스 연습 중"
        } else {
            "코스로 이동 중"
        }
        val message = if (isInCourseScope) {
            progress?.let { "${session.placeName} · 연습 진행률 $it%" }
                ?: "${session.placeName}의 코스 연습을 확인하고 있어요."
        } else {
            "${session.placeName}에 도착하면 코스 연습을 기록해 드릴게요."
        }
        ...
        val builder = NotificationCompat.Builder(context, ONGOING_CHANNEL_ID)
            ...
            .addAction(
                R.drawable.ic_notification_rodi,
                "운전 종료",
    ...
    fun arrival(
        ...
        .setStyle(
            NotificationCompat.BigTextStyle()
                .bigText("안전한 장소에 정차한 후 운전 기록을 확인해 주세요."),
        )
        .setContentIntent(openAppIntent(context))
        .setAutoCancel(false)
        .setOngoing(true)
