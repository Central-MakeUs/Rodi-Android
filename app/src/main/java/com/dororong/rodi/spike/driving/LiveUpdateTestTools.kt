...
internal object LiveUpdateTestTools {
    ...
    private fun diagnostics(context: Context): String = buildString {
        ...
        appendLine("OS 36.1 이상: ${yesNo(osSupported)}")
        appendLine("사용자 허용: ${yesNo(userAllowed)}")
        ...
        if (!osSupported) appendLine("→ 이 OS 버전은 실시간 업데이트 표시를 지원하지 않아요.")
        if (!userAllowed) appendLine("→ 시스템 앱 설정에서 이 앱의 실시간 업데이트가 꺼져 있어요.")
        appendLine()
        appendLine("[표시 중인 테스트 알림]")
        appendLine("시스템 승격: ${previewPromotionText(context)}")
        if (Build.MANUFACTURER.equals("samsung", ignoreCase = true)) {
            appendLine()
            appendLine("[참고] One UI 8은 개발자 옵션 '모든 앱의 실시간 알림'이 꺼져 있으면 삼성 허용 목록 앱만 표시해요. 위 조건이 모두 '예'인데 안 보이면 이 경우예요.")
        }
    }.trimEnd()
