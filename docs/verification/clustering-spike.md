...
- 지역 클러스터에서 반 화면 이상 이동 시 `현 지도에서 검색` 버튼을 150ms 페이드인하고, 탭 전 지역 조회·클러스터 갱신 보류
...
- 데이터: 기존 `SampleCourses.RODI_COURSES`와 서울권 N-퀸 배치 40건의 로컬 더미 데이터(고정 대한민국 범위 조회 시 191개)
...
| 줌 | 정책 | 관찰 결과 | 증거 |
|---:|---|---|---|
| 7 | 고정 3×5 전국 클러스터 | 기존 더미 데이터로 재검증 필요 | - |
| 10 | 전국 캐시 재사용 | N-퀸 추가 전 151개·10개 점유 셀 | `rodi-sample-courses-default.png` |
| 11 | 화면 거리 기반 지역 클러스터 | 56dp 안의 연결된 마커를 하나로 표시 | 기기 재확인 필요 |
| 13 | 화면 거리 기반 지역 클러스터 | 숫자 클러스터·단건 핀의 최소 간격 보장 | 기기 재확인 필요 |
...
- 7~10의 3×5는 고정 대한민국 범위(NE 39.3, 131.8 / SW 32.7, 124.4)의 기존 더미 데이터 캐시로 한 번만 계산한다. 카메라 확대·이동으로 viewport 조회가 발생해도 전국 클러스터의 멤버·개수·셀 경계는 다시 계산하거나 다시 렌더링하지 않는다.
- 기존 더미 데이터 연결 후 줌 10에서 151개·10개 점유 셀이 표시됐고, 이후 서울권 N-퀸 배치 40건을 추가했다. 증거: `rodi-sample-courses-default.png`.
- 11~13은 마커 중심 간 화면 거리 56dp를 기준으로 연결된 멤버를 하나의 클러스터로 묶는다. 서로 다른 이전 그리드 셀에 있더라도 가까우면 합쳐지므로 숫자 클러스터·단건 핀이 겹치지 않는다. 한 건만 남은 그룹은 시작·주차장 핀으로 표시한다.
...
- 마지막 검색 viewport의 중심과 현재 viewport의 중심 사이를 가로·세로 반 화면 이동량으로 정규화해 합산 거리가 1.0 이상일 때만 재검색 버튼을 노출한다. 작은 이동은 버튼과 조회 모두 발생시키지 않는다.
- 에뮬레이터에서 지역 줌 13의 두 번째 충분한 거리 드래그 후 버튼 노출, 탭 후 숨김을 확인했다. 증거: `rodi-search-visible.png`, `rodi-search-after-tap.png`.
- 현재 3×5 고정 전국 그리드 / 56dp 지역 반경 및 7~10 / 11~13 / 14+ 경계는 기술 검증안이다. 실제 서버 데이터 밀도와 최종 마커 크기가 확정되면 지역 반경을 다시 측정해 조정하는 편이 안전하다.
...
- 더미 코스의 대표 좌표는 출발지이며 실제 운영 데이터 분포와 다를 수 있다.
