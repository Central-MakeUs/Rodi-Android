...
- [ ] `PermissionSettingsScreen`의 행 문구가 사용자가 무엇을 얻는지 설명하지 못한다.
  "주행 상태 알림"은 설명이 있지만 "실시간 업데이트"는 없었고(그래서 이번에 행 자체를 조건부로
  숨겼다), "위치"·"주행 상태 알림"도 허용/미허용만 보여 줄 뿐 거부했을 때 무엇이 안 되는지
  알려주지 않는다. **기획과 함께 문구를 다시 쓴다.**
- 배경: One UI 8 실기기에서 "실시간 업데이트"가 항상 "미허용"으로 보이는데 삼성 설정에는 켤
  스위치가 없어 사용자가 할 수 있는 일이 없었다. 지금은 앱별 프로모션 설정 화면을 제공하는
  기기에서만 행을 노출한다(`supportsPromotedNotificationSettings()`).
