...
- [ ] **순환 코스 마커 앵커 Y 값 재평가 (2026-08-14)** — 출발==도착이 겹칠 때만
  `LabelStyle.setAnchorPoint(x, 1f)`를 준다(`CourseRouteRenderer.PIN_ANCHOR_Y`). 그런데 Kakao SDK
  기본 앵커는 `PointF(0.5f, 0.5f)`라(바이트코드 확인) **겹치는 경우에만 수직 기준이 0.5→1.0으로
  같이 바뀌어**, 같은 화면의 경유 핀·비순환 코스 핀과 정렬 기준이 달라진다. 좌우 분리만이 목적이니
  Y는 기본값 유지가 맞다. `0.5f`로 바꿔 실측 비교하려 했으나 재설치 후 추천 목록이 빈 상태로 걸려
  동일 조건 캡처를 못 얻었고, **검증 못 한 변경을 남기지 않으려 되돌렸다.** 현재 `1f`는 5회 반복
  AVD 검증을 통과한 상태다. AVD가 안정적일 때 두 값을 같은 조건에서 비교해 결정할 것.
