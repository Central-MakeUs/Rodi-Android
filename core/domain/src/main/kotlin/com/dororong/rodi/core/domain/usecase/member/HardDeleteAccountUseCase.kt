...
class HardDeleteAccountUseCase @Inject constructor(
    ...
    suspend operator fun invoke(): Result<HardDeleteResult> = runSuspendCatching {
        val result = memberRepository.hardDelete()
        var localCleanupSucceeded = result.localCleanupSucceeded
        try {
            clearOnboardingData()
        ...
        result.copy(localCleanupSucceeded = localCleanupSucceeded)
    }
}
